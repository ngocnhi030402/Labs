...
void StackFrame::run(string filename) {
    ...
    while (getline(file, s)){
        ...
        if(s.compare("f2i") == 0 || s.compare("i2f") == 0){
         ...
         //cout << s2 << ' ' << s1 <<endl;
        // cout << "length: " << s2.length() << ", " << s2.length() << endl;
        ...
        if (s1 != ""){
            if (s2.compare("iconst") == 0){
                ...
                //cout << stoi(s1) << endl;
                //cout << ind_arr[stoi(s1)] << endl;
                //if(count == 256) throw UndefinedVariable(linee);
                
                if(ind_arr[stoi(s1)] == -999999) throw UndefinedVariable(linee);
                ...
                if(stoi(s1) >= 256) throw ArrayOutOfRange(linee);
                 if(line == 32) throw StackFull(linee);
                 //cout << "stackk[line - 1]: " << stackk[line - 1] << endl;
                if(ind_arr[stoi(s1) + 1] == 0){
                    throw TypeMisMatch(linee);
                }
                if(stackk[line - 1] != 0){
                    StackFrame::fload(s1);
...
void StackFrame::istore(string s){
    int idx = stoi(s);
    //cout << "stackk[line - 2]: " << stackk[line - 2] << endl;
    arr[idx] = stackk[line - 2];
    
    ind_arr[idx] =stackk[line - 2];
    //cout << "ind_arr[idx]: " << ind_arr[idx] << endl;
...
void StackFrame::iload(string s){
   ...
   //cout << "stackk[line]: " << stackk[line] << endl; 
    line++;
    stackk[line] = 0;
    line++;
}
...
void StackFrame::disp_arr(float * arr, int size){
